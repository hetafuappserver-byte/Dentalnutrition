  ...
  return (
    <main id="main" className="bg-[#fcfcfc] text-[#401e17]">
      <section className="relative pb-10 md:pb-14">
        <div
          className="banner__image w-full h-[580px] bg-cover bg-center block relative"
        ...
        <div className="relative z-[1] mx-auto grid min-[900px]:grid-cols-[minmax(0,0.32fr)_minmax(0,0.38fr)] min-[800px]:items-start min-[800px]:gap-60 -mt-80 ml-40">
          <div>
            <div className="aspect-square rounded-full bg-[#f3f3f3] overflow-hidden [&_img]:w-full [&_img]:h-full [&_img]:object-contain">
          ...
          <div className="min-[1000px]:text-left mt-87 ml-[-70px]">
          ...
          <div className="flex justify-center gap-8 border-b border-black/[0.08] mb-8" role="tablist">
            {TAB_DEFINITIONS.map((tab) => (
              <button
                ...
                className={`bg-transparent border-0 border-b-2 cursor-pointer font-[Instrument_Sans,sans-serif]    tracking-[0.12em] uppercase py-4 text-lg mx-2 -mb-px text-inherit ${activeTab === tab.id ? "opacity-100 border-b-[#401e17]" : "opacity-55 border-transparent"}`}
      ...
      <section className="max-w-[1200px] mx-auto px-5 pb-12 md:px-8 md:pb-16" aria-label="Products">
        <div className="flex justify-end py-3 pb-6 border-t border-black/[0.08]">
          <p className="m-0 text-sm tracking-[0.04em] opacity-65 lowercase">
            {LOLLIPOPS_PRODUCTS.length} products
          </p>
        </div>

        <div className="grid grid-cols-2 gap-5 max-w-[1200px] mx-auto md:grid-cols-4 md:gap-6">
          {LOLLIPOPS_PRODUCTS.map((product) => (
            <Link key={product.slug} href={getProductHref("lollipops", product.slug)} className="text-center">
              <div className="aspect-square bg-[#f3f3f3] mb-3 overflow-hidden [&_img]:w-full [&_img]:h-full [&_img]:object-contain">
