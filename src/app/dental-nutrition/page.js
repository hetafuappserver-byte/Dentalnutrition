...
const IMAGES = {
  ...
  overlay4: "/DentalNutrition/overlay4.png",
...
const OVERLAY_SECTIONS = [
  ...
  {
    title: "Heart Health Impact",
    paragraphs: ["🫀 70% of Heart attacks — Harmful oral bacteria and gum inflammation can enter the bloodstream, damage blood vessels, promote arterial plaque buildup, and increase the risk of clot formation."],
    image: IMAGES.overlay3,
  },
  {
    title: "Lung Health Impact",
    paragraphs: ["🫁 Lung infections — Oral bacteria can be aspirated or spread systemically, increasing the risk and severity of respiratory infections, including pneumonia and chronic bronchitis."],
    image: IMAGES.overlay2,
  },
  {
    title: "Cognitive Health Impact",
    paragraphs: ["🧠 Brain fog — Oral microbial imbalance impairs cognitive function (focus, memory, and mental ability)."],
    image: IMAGES.overlay3,
  },
  {
    title: "Digestive Health Impact",
    paragraphs: ["🦠 IBS & bloating — Harmful oral bacteria can travel to the gut, alter gut microbial balance, and promote intestinal inflammation linked with IBS and related digestive disorders."],
    image: IMAGES.overlay2,
  },
  {
    title: "Kidney Health Impact",
    paragraphs: ["🫘 Kidney infections — Harmful oral bacteria can travel through the blood and overload the kidneys."],
    image: IMAGES.overlay3,
  },
  {
    title: "Cancer Risk Impact",
    paragraphs: ["🧬 Cancers — Long-term oral microbial imbalance can damage DNA (Breast, lung, stomach)."],
    image: IMAGES.overlay2,
  },
  {
    title: "Diabetes Impact",
    paragraphs: ["🩸 Diabetes — Increases insulin resistance."],
    image: IMAGES.overlay3,
  },
  {
    title: "Pregnancy Health Impact",
    paragraphs: ["🤰 Pregnancy complications — Harmful oral bacteria increase the risk of early birth and low birth weight."],
    image: IMAGES.overlay2,
  },
  {
...
function ImageTextOverlay({ title, paragraphs, image, align = "left" }) {
  ...
  return (
    <section className="relative w-full overflow-hidden" aria-label={title}>
      <div className="relative w-full">
        <Image src={image} alt="" width={2000} height={1200} className="block h-auto min-h-[280px] w-full object-cover min-[700px]:min-h-[420px]" />
        <div className={`absolute inset-0 flex items-center bg-black/30 px-5 py-10 md:px-12 md:py-14 lg:px-20 ${isRight ? "justify-end" : "justify-start"} max-md:justify-center`}>
          <div className={`max-w-[680px] text-white max-md:text-center ${isRight ? "text-right" : "text-left"}`}>
            <p className="m-0 mb-4 [font-family:var(--heading-font)] text-[clamp(1.25rem,3vw,1.75rem)] font-normal tracking-[0.1em] uppercase">{title}</p>
    ...
    <>
      ...
      <main id="main" className="bg-[#fcfcfc] text-[#401e17]">
        <section className="w-full overflow-hidden" aria-label="Dental Nutrition hero">
          <Image src={IMAGES.hero} alt="Dental Nutrition" width={2000} height={1200} priority className="block h-auto min-h-[280px] w-full object-cover min-[700px]:min-h-[420px]" />
        </section>

        <section className="border-b border-[rgba(64,30,23,0.15)] px-5 py-10 md:px-8 md:py-16">
          <div className="mx-auto max-w-[680px] text-center">
            <p className="m-0 mb-5 [font-family:var(--heading-font)] text-[clamp(1.75rem,4vw,2rem)] font-normal tracking-[0.1em] uppercase">Dental Nutrition</p>
            <div className="[&_p]:m-0 [&_p]:mb-4 [&_p]:text-lg [&_p]:leading-[1.65] [&_p:last-child]:mb-0">
              <p>In simple words, Dental nutrition is oral care in edible form.</p>
              <p>Medicines can give temporary relief from internal disease conditions, but when the mouth (the body&apos;s main entry point) is left unprotected, harmful bacteria continue to enter the bloodstream every day, causing these problems to recur.</p>
              <p>Just as a country needs soldiers at its borders, the mouth needs gatekeepers that stop repeated bacterial attacks at the source, reduce disease recurrence, protect vital organs, and support long-term overall health. This is exactly where Hetafu steps in.</p>
            </div>
          </div>
        </section>
        ...
        <div className="flex flex-col gap-6 py-10 md:gap-10 md:py-14">
          {OVERLAY_SECTIONS.map((section, index) => (
            <ImageTextOverlay key={section.title} title={section.title} paragraphs={section.paragraphs} image={section.image} align={index % 2 === 0 ? "right" : "left"} />
          ))}
        </div>
        ...
        <section className="border-t border-[rgba(64,30,23,0.15)] px-5 py-10 md:px-8 md:py-16">
          <div className="mx-auto grid max-w-[1200px] grid-cols-1 items-center gap-8 md:grid-cols-2">
            <div className="flex justify-center">
              <Image src={IMAGES.appPhone} alt="Dental Nutrition app" width={780} height={900} className="h-auto w-full max-w-[430px] object-contain" />
            </div>
            <div className="text-center">
              <h3 className="m-0 mb-4 [font-family:var(--heading-font)] text-[clamp(1.25rem,3vw,1.875rem)] font-normal tracking-[0.05em]">
                <strong>Ask our Dental Nutrition Experts about all things dental nutrition!</strong>
              </h3>
              <p className="m-0 mb-6 text-lg leading-[1.65]">
                Access inspirational and educational videos, plus self-guided learning on nutrition, dental health, product application, and more. Available for iOS and Android mobile and tablet devices.
              </p>
              <div className="flex flex-wrap justify-center gap-4 [&_img]:h-11 [&_img]:w-auto">
                <a href="https://apps.apple.com/us/app/dental-nutrition/id123456789">
                  <Image src={IMAGES.appStore} alt="Download on the App Store" width={160} height={44} />
                </a>
                <a href="https://play.google.com/store/apps/details?id=com.dentalnutrition">
                  <Image src={IMAGES.googlePlay} alt="Get it on Google Play" width={160} height={44} />
                </a>
              </div>
            </div>
          </div>
